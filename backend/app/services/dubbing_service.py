...
def generate_dub_background(
    ...
    try:
        ...
        dubbing_tasks[task_id]["audio_url"] = f"/{dubbed_audio_path.replace(os.sep, '/')}"
        ...
        output_movie = os.path.join(task_dir, f"movie_{safe_lang}.mp4")
        ...
        dubbing_tasks[task_id]["movie_url"] = f"/{output_movie.replace(os.sep, '/')}"
        dubbing_tasks[task_id]["step_progress"] = 100
